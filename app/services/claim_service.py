...
def create_claim(data: ClaimCreate, current_user: User, db: Session) -> dict:

    try:

        logger.info(f"Creating claim : policy {data.policy_id}")
        ...
        audit_repo.log(
            ...
            entity_type="Claim",
            entity_id=claim.id,
            description=f"Claim {claim_number} created for policy {data.policy_id}"
        )
        ...
        logger.info(f"Claim created successfully : {claim.id}, status DRAFT")
...
def get_claim_by_id(claim_id: int, db: Session) -> dict:
    ...
    logger.info(f"Fetching claim by id : {claim_id}")
...
def get_all_claims(
    ...
    logger.info("Fetching claims list.")
    ...
    claims, total_records = claim_repo.list_claims(
        claim_status, claim_type, start_date, end_date, min_amount, max_amount, sort_column, sort_order, get_offset(page, limit), limit
    )

    return {"message": "Claims fetched successfully.", "data": claims, "pagination": get_pagination(total_records, page, limit)}


def update_claim(claim_id: int, data: ClaimUpdate, current_user: User, db: Session) -> dict:
...
def submit_claim(claim_id: int, current_user: User, db: Session) -> dict:

    try:

        logger.info(f"Submitting claim : {claim_id}")
...
def approve_claim(claim_id: int, current_user: User, db: Session) -> dict:
    ...
    try:
        ...
        logger.info(f"Claim approved successfully : {claim_id}")
...
def reject_claim(claim_id: int, current_user: User, db: Session) -> dict:

    try:

        logger.info(f"Rejecting claim : {claim_id}")
        ...
        logger.info(f"Claim rejected successfully : {claim_id}")
